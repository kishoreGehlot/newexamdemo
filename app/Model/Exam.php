...
class Exam extends AppModel
{
    ...
    public function getUserExam($type, $studentId, $currentDateTime, $limit = 0)
    {
        ...
        if ($type == "today") {
            ...
            $expiredDate = "HAVING (`Exam__fexpiry_date` >='$currentDate' OR `Exam__fexpiry_date` IS NULL) OR Exam.expiry IS NULL";
        }
        if ($type == "upcoming") {
            $start_date = array('Exam.start_date >' => $currentDateTime);
        }
        ...
        $this->virtualFields = array('attempt' => 'SELECT COUNT(ExamResult.id) FROM `exam_results` AS `ExamResult` WHERE `ExamResult`.`exam_id`=`Exam.id` AND `ExamResult`.`student_id`=' . $studentId,
            ...
            'fexpiry_date' => 'SELECT Max(ExamOrder.expiry_date) FROM `exam_orders` AS `ExamOrder` WHERE `ExamOrder`.`exam_id`=`Exam.id` AND `ExamOrder`.`student_id`=' . $studentId);
        $examList = $this->find('all', array(
            'fields' => array('Exam.id', 'Exam.fexpiry_date', 'Exam.type', 'Exam.name', 'Exam.start_date', 'Exam.end_date', 'Exam.paid_exam', 'Exam.amount', 'Exam.attempt', 'Exam.attempt_count', 'Exam.attempt_order', 'Exam.expiry', 'ExamOrder.expiry_date', 'ExamOrder.date'),
            'joins' => array(array('table' => 'exam_questions', 'alias' => 'ExamQuestion', 'type' => 'LEFT','conditions' => array('Exam.id=ExamQuestion.Exam_id')),
                array('table' => 'exam_groups', 'alias' => 'ExamGroup', 'type' => 'Inner','conditions' => array('Exam.id=ExamGroup.exam_id', '')),
                array('table' => 'student_groups', 'alias' => 'StudentGroup', 'type' => 'LEFT','conditions' => array('StudentGroup.group_id=ExamGroup.group_id')),
                array('table' => 'questions', 'alias' => 'Question', 'type' => 'LEFT','conditions' => array('ExamQuestion.question_id=Question.id')),
                array('table' => 'exam_preps', 'alias' => 'ExamPrep', 'type' => 'LEFT','conditions' => array('Exam.id=ExamPrep.exam_id')),
                array('table' => 'exam_orders', 'alias' => 'ExamOrder', 'type' => 'LEFT','conditions' => array('Exam.id=ExamOrder.exam_id'))),
    ...
    public function getPurchasedExam($type, $studentId, $currentDateTime, $limit = 0)
    {
        ...
        if ($type == "today") {
            $start_date = array('Exam.start_date <=' => $currentDateTime);
            $end_date = array('Exam.end_date >' => $currentDateTime);
            $purchaseCond=array('Exam.paid_exam'=>1);
        }
        if ($type == "upcoming") {
            $start_date = array('Exam.start_date >' => $currentDateTime);
        }
        if ($type == "expired") {
            $start_date = array('Exam.start_date <=' => $currentDateTime);
            $end_date = array('Exam.end_date >' => $currentDateTime);
            $expiredDate = "HAVING Max(ExamOrder.expiry_date) <'$currentDate'";
        }
        if ($limit > 0)
            $examLimit = $limit;
        ...
        $examList = $this->find('all', array(
            'fields' => array('Exam.id', 'Exam.fexpiry_date', 'Exam.name', 'Exam.start_date', 'Exam.end_date', 'Exam.paid_exam', 'Exam.amount', 'Exam.type', 'ExamOrder.expiry_date', 'ExamOrder.date', 'Exam.attempt', 'Exam.attempt_count', 'Exam.attempt_order', 'Exam.expiry'),
            'joins' => array(array('table' => 'exam_questions', 'alias' => 'ExamQuestion', 'type' => 'LEFT',
                ...
                array('table' => 'questions', 'alias' => 'Question', 'type' => 'LEFT','conditions' => array('ExamQuestion.question_id=Question.id')),
                array('table' => 'exam_preps', 'alias' => 'ExamPrep', 'type' => 'LEFT','conditions' => array('Exam.id=ExamPrep.exam_id')),
                array('table' => 'exam_orders', 'alias' => 'ExamOrder', 'type' => 'LEFT','conditions' => array('Exam.id=ExamOrder.exam_id', 'StudentGroup.student_id=ExamOrder.student_id'))),
            'conditions' => array($start_date, $end_date,$purchaseCond, "StudentGroup.student_id=$studentId", 'Exam.status' => 'Active', 'Exam.user_id' => 0, 'ExamOrder.student_id' => $studentId),
            'order' => array('Exam.start_date' => 'asc'),
            'group' => array("Exam.id $expiredDate"),
            'limit' => $examLimit));
        return $examList;
    ...
    public function userExamQuestion($exam_id, $studentId, $quesNo = 0, $type = "first")
    {
        $condition = array();
        $ExamStat = ClassRegistry::init('ExamStat');
        ...
        if ($examQuestionCount == 0)
            $quesNo = 1;
        if ($type == "first") {
            $condition = array('ExamStat.ques_no' => $quesNo);
        }
        $userExamQuestion = $ExamStat->find($type, array('fields' => array('ExamStat.id', 'ExamStat.correct_answer', 'ExamStat.exam_result_id', 'ExamStat.answered', 'ExamStat.ques_no', 'ExamStat.option_selected', 'ExamStat.true_false', 'ExamStat.fill_blank',
            'ExamStat.marks', 'ExamStat.review', 'ExamStat.options','ExamStat.opened',
            ...
            'conditions' => array('exam_id' => $exam_id, 'student_id' => $studentId, 'closed' => 0, $condition),
    ...
    public function userSaveAnswer($exam_id, $quesNo, $studentId, $currentDateTime, $valueArr)
    {
        ...
        if ($maxAnswer >= $maxQuestion && $maxQuestion != 0) {
            ...
            if (isset($valueArr['Exam']['attempt_time'])) {
            ...
            //if (isset($valueArr['Exam']['review'])) {
            //    $usrQues['ExamStat']['review'] = $valueArr['Exam']['review'];
            //}
    ...
    public function userReviewAnswer($exam_id, $quesNo, $studentId, $review)
    {
        ...
        $id = $currRecord['ExamStat']['id'];
        $usrQues = array("ExamStat" => array('id' => $id, 'review' => $review));var_dump($usrQues);
        if($ExamStat->save($usrQues)){
            echo"Yes";
        }else{
            echo"No";
        }
