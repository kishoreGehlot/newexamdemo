...
class FunctionHelper extends Helper
{
    ...
    public function showExamList($showType, $exam, $currency, $dateFormat, $frontExamPaid, $examExpiry)
    {
        ...
        $expireValue = null;
        $dateHeading = null;
        if ($showType == 'today' || $showType == 'purchased') {
            $ppendingAttempt = "<th>" . __('Attempts') . "<br>" . __('Remaining') . "</th>";
        }
        ...
        if ($showType == 'expired') $dateHeading .= "<th>" . __('Expiry Date') . "</th>";
        ...
        foreach ($exam as $post) {
            ...
            if ($frontExamPaid == true) {
                if ($post['Exam']['paid_exam'] == "1") {
                    $amount = "<td>$currency" . $post['Exam']['amount'] . "</td>";
                } else {
                    $amount = "<td>" . __('Free') . "</td>";
                }
            }
            if ($post['Exam']['attempt_count'] == 0) {
                $pendingAttempt = __('Unlimited');
            } else {
                if ($post['Exam']['paid_exam'] == 1 && !$post['ExamOrder']['date']) {
                    $pendingAttempt = __('Not Purchased');
                } else {
                    if ($post['Exam']['paid_exam'] == 1) $pendingAttempt = ($post['Exam']['attempt_order'] * $post['Exam']['attempt_count'] - $post['Exam']['attempt']); else$pendingAttempt = ($post['Exam']['attempt_count'] - $post['Exam']['attempt']);
                }
            }
            if ($showType == 'today' || $showType == 'purchased') {
            ...
            if ($showType == 'purchased') {
                if ($post['Exam']['expiry'] == 0) $dateValue = "Unlimited"; else$dateValue = CakeTime::format($dateFormat, $post['Exam']['fexpiry_date']);
            }
            ...
            if ($showType != 'expired' && $showType != 'purchased' && $examExpiry) {
                if ($post['Exam']['expiry'] == 0) {
                    $expireValue = '<td>' . __('Unlimited') . '</td>';
                } else {
                    if ($post['Exam']['fexpiry_date']) {
                        $expireValue = '<td>' . CakeTime::format($dateFormat, $post['Exam']['fexpiry_date']) . '</td>';
                    } else {
                        $expireValue = '<td>' . $post['Exam']['expiry'] . __('Days') . '</td>';
                    }
                }
            }
            if ($showType == 'purchased' && $examExpiry) {
                if ($post['Exam']['expiry'] == 0) {
                    $expireValue = '<td>' . __('Unlimited') . '</td>';
                } else {
                    $expireValue = '<td>' . CakeTime::format($dateFormat, $post['Exam']['fexpiry_date']) . '</td>';
